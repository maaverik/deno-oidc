...
auth.get("/callback", async (req, res) => {
 ...
 if (req.query.state.split(":")[0] !== state) {     // To make sure the registered IdP is the one who sent auth code
   res.send("State code does not match.").sendStatus(400);
 }

 const tokenUrl: string = `${issuer}/v1/token`;
 const code: string = req.query.code;       // auth code
 ...
 const user = parseJwt(data.id_token);      // id token
